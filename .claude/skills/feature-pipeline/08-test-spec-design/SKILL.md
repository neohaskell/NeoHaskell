...
1. **Produce** — spawn an Agent (model: sonnet) and instruct it to read `./01-produce/SKILL.md` and follow it against the architecture doc at `docs/architecture/<adr-number>-<slug>.md`. Verify: `docs/architecture/<adr-number>-<slug>-tests.md` exists.
...
3. After the review-quality step records `pass`, the orchestrator runs `python3 .claude/skills/feature-pipeline/scripts/pipeline.py complete 8`. If the rubric records `fail`, the agent has already refused — the orchestrator stops the pipeline at this step.
