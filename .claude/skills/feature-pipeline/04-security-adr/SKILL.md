...
1. **Threat model** — spawn an Agent (model: sonnet) and instruct it to read `./01-threat-model/SKILL.md` and follow it on the ADR at `docs/decisions/<adr>-<slug>.md`. Verify: stdout returns a JSON array of raw findings with `severity` and `rule` fields.
