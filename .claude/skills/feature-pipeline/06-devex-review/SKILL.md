...
1. **Produce** — spawn an Agent (model: sonnet) and instruct it to read `./01-produce/SKILL.md` and follow it on the ADR at `docs/decisions/<adr-number>-<slug>.md` and the findings at `.pipeline/findings-04.json` + `.pipeline/findings-05.json`. Verify: `.pipeline/devex-review.md` exists and contains a `## Per-criterion verdicts` section.
...
3. After the review-quality step records `pass`, the orchestrator runs `python3 .claude/skills/feature-pipeline/scripts/pipeline.py complete 6`. If the rubric records `fail`, the agent has already refused — the orchestrator stops the pipeline at this step.
