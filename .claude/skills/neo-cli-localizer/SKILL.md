...
For a CLI-only task, do not explore the Haskell `core/`, `testbed/`, or
`integrations/` trees. For an atomic cross-component task, use the NeoHaskell
localizer for the Haskell side and keep each concern in the matching adjacent
stack layer; do not infer NeoHaskell APIs from the Rust implementation.
