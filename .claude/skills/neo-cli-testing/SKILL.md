...
source revision (generated by `./dev neo-release compat` from the starter pins —
see `neo/AGENTS.md`). The consumer contract's **phase 2b** gates it: after
`neo new`, it runs `neo-release compat` (which fails closed on any drift among
`neo/starter/` `flake.nix` / `flake.lock` / `cabal.project`) and asserts the
freshly generated project pins exactly the declared `source_revision`. If you
change the starter's pinned NeoHaskell revision, bump all three pins together or
this gate (and `neo-release --self-test`) goes red — that red bar means the
published contract would misdescribe what `neo new` produces. Do not weaken it.
