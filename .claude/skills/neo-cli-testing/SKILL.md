...
Four layers, each a specific invocation. **All `cargo` commands run inside
`nix develop --command`** (the toolchain is flake-pinned; a bare `cargo` fails with
`ld: library not found for -liconv`). `nix build` is nix-CLI and runs outside the
dev shell.

| Layer | Binary under test | Command | Network / Nix | Time |
|---|---|---|---|---|
| Unit | in-crate functions, parsers, TUI widgets, prereqs guards | `nix develop --command cargo test --bins` | none | seconds |
| Integration | `cargo`-built debug `neo` via `cargo_bin("neo")` | `nix develop --command cargo test --test integration_tests -- --test-threads=1` | real nix + real network | minutes (Haskell builds inside) |
| E2E | the **nix-built** `result/bin/neo` (what a user installs) | `nix build && nix develop --command cargo test --test e2e -- --ignored --test-threads=1 --nocapture` | real nix + real network | minutes |
| Neo-on-Neo smoke | the binary driven over a generated project | manual: `neo new` a project, then `neo build` + `neo test` inside it | real nix + real network | minutes |
...
- E2E requires a fresh `result/bin/neo`; run `nix build` first (the helper panics
...
1. **Starter ↔ upstream drift.** `neo new` tarballs `neo-starter@main`; `neo build`
...
Either way these red bars are the intended signal that the starter↔upstream
...
`nix develop --command cargo clippy` is clean, and — if the generated project
