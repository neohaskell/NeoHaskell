...
## Toolchain — always inside `nix develop`

The host provides no `cargo`/`rustc`. Wrap every toolchain command:
...
Full stack (Rust binary + embedded IDE `dist/`): `nix develop --command ./scripts/build.sh [--release]`.
The telltale of skipping the wrapper is `ld: library not found for -liconv` — the
fix is always "wrap in `nix develop --command`", never "install iconv". `nix build`
and `nix flake check` are nix-CLI and run *outside* the dev shell.
