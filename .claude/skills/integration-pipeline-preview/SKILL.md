...
3. **Design draft 🔒** — spawn an Agent (model: opus) and instruct it to read `./03-design-draft/SKILL.md` and follow it. Verify: `.integration-pipeline/integration-design.md` exists with Status: Proposed. Then `pipeline.py complete 3` and stop until `pipeline.py approve 3`.
