#!/usr/bin/env python3
"""
NeoHaskell integration pipeline — deterministic state machine.

Reads/writes .integration-pipeline/state.json under the project root. Each
phase has a status (pending | in_progress | completed), an optional
approved_at timestamp, and tracks its findings file (for the security/perf
phases). Phase numbers, names, dependencies, and PAUSE gates are defined
in PHASES.

This is the integration-pipeline-preview variant of the feature pipeline.
The design doc is never written to docs/decisions/ (those slots are
reserved for repo-level ADRs); the integration design doc lives in
.integration-pipeline/integration-design.md instead so the integration
can later be moved to a separate repo without orphaning an ADR record.

Commands:
  init <integration> [options]  Initialise the pipeline state.
  ...
  classify <tier> <rationale>   Persist the integration complexity tier.
  ...
  set <key> <value>             Set a variable (integration_name, branch_name, ...).
...
PIPELINE_DIR = ".integration-pipeline"
...
PHASES: dict[int, dict[str, Any]] = {
    ...
    2:  {"name": "Classify integration",     "depends": [1],     "pause": False},
    3:  {"name": "Design draft",             "depends": [2],     "pause": True},
    4:  {"name": "Security review (design)", "depends": [3],     "pause": False},
    5:  {"name": "Performance review (design)","depends": [3],   "pause": False},
...
def slugify(name: str) -> str:
    """Lower-case + hyphenate, used to derive the integration module slug."""
...
def load_state() -> dict[str, Any]:
    if not os.path.exists(STATE_FILE):
        die("no pipeline initialised — run: pipeline.py init <integration>", 2)
    lock = _acquire_lock()
    try:
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    slug = slugify(args.integration)
    # The Pascal-case module suffix mirrors `integrations/Integration/<Name>.hs`
    # (e.g. integration_name="oura health" -> module_name="Oura"). Take the
    # first word so the common single-vendor case lands the bare vendor name;
    # multi-word integrations can override via --module-name.
    parts = [p for p in re.split(r"[\s\-_]+", args.integration.strip()) if p]
    default_module_name = parts[0].capitalize() if parts else ""
    state = {
        "version": 1,
        "created_at": now_iso(),
        "updated_at": now_iso(),
        "variables": {
            ...
            "branch_name": args.branch or f"integration/{slug}",
            "module_name": args.module_name or default_module_name,
            "module_path": args.module or "",
            "test_path": args.test or "",
            # Design artefacts live under .integration-pipeline/, NEVER under
            # docs/decisions/. The whole point of this pipeline is that the
            # produced integration is portable to a separate repo, and an
            # ADR slot in this repo would orphan if/when the integration moves.
            "design_path": os.path.join(PIPELINE_DIR, "integration-design.md"),
            "architecture_path": os.path.join(
                PIPELINE_DIR, "integration-architecture.md"
            ),
            "test_spec_path": os.path.join(PIPELINE_DIR, "integration-tests.md"),
            "pr_number": "",
            "pr_url": "",
        },
        "phases": {
    ...
    print(f"Pipeline initialised for: {args.integration}")
    print(f"  Branch:       {state['variables']['branch_name']}")
    print(f"  Module name:  {state['variables']['module_name'] or '(unset)'}")
    print(f"  Module path:  {state['variables']['module_path'] or '(unset)'}")
    print(f"  Test path:    {state['variables']['test_path'] or '(unset)'}")
    print(f"  Design doc:   {state['variables']['design_path']}")


def cmd_status(args: argparse.Namespace) -> None:
    state = load_state()
    v = state["variables"]
    print(f"\nintegration-pipeline-preview: {v['integration_name']}")
    bits = []
    if v.get("issue_number"): bits.append(f"issue #{v['issue_number']}")
    if v.get("branch_name"): bits.append(f"branch {v['branch_name']}")
    if v.get("module_name"): bits.append(f"module {v['module_name']}")
    if bits:
        print("  " + " | ".join(bits))
    print()
    print(f"{'#':>3}  {'phase':<32}  {'status':<18}")
    print(f"{'-'*3}  {'-'*32}  {'-'*18}")
...
def _phase_dir(num: int) -> str:
    ...
    names = {
        1:  "01-init-pipeline",
        2:  "02-classify-integration",
        3:  "03-design-draft",
        4:  "04-security-design",
        5:  "05-performance-design",
        6:  "06-devex-review",
    ...
    try:
        ...
        for fname in ("state.json", "classification.json", ".lock"):
            ...
            if os.path.exists(path):
                os.remove(path)
        # Findings files and design artefacts may have been written elsewhere
        # under .integration-pipeline/ — leave them for the maintainer.
...
def main() -> None:
    ...
    p.add_argument("integration",
                   help="Human-readable integration name (e.g. 'Stripe webhooks')")
    p.add_argument("--issue", help="GitHub issue number")
    p.add_argument("--module-name",
                   help="Module suffix under integrations/Integration/<Name> "
                        "(default: capitalised first word of <integration>)")
    p.add_argument("--module",
                   help="Main module path (e.g. integrations/Integration/Stripe.hs)")
    p.add_argument("--test",
                   help="Test file path (e.g. integrations/test/StripeSpec.hs)")
    p.add_argument("--branch", help="Git branch name (default: integration/<slug>)")
    ...
    p = sub.add_parser("classify", help="Persist the integration complexity tier")
    p.add_argument("tier", help=f"one of: {sorted(VALID_TIERS)}")
    p.add_argument("rationale",
                   help="One-line justification (e.g. 'outbound HTTP, no trust boundary')")

    p = sub.add_parser("findings", help="Register a findings file for a phase")
    p.add_argument("phase", type=int)
    p.add_argument("path")

    p = sub.add_parser("iter",
                       help="Increment & print this phase's iteration counter")
    p.add_argument("phase", type=int)
