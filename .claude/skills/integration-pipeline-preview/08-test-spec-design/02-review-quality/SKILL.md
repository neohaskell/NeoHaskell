...
Reads the produced test spec and the architecture doc, applies every check in `../../references/test-spec-rubric.md`, records the per-check verdict to `.integration-pipeline/test-spec-rubric.json`, and gates phase 8 completion on the result.
...
- `.integration-pipeline/integration-tests.md` — produced by `../01-produce/`.
- `.integration-pipeline/integration-architecture.md` — the source architecture doc.
...
4. Write the rubric record to `.integration-pipeline/test-spec-rubric.json` → verify: file exists with `checks`, `carriers`, `verdict`.
...
5. Write `.integration-pipeline/test-spec-rubric.json`.
6. On `pass`: run `python3 .claude/skills/integration-pipeline-preview/scripts/pipeline.py complete 8`; print `RUBRIC: pass`.
...
`.integration-pipeline/test-spec-rubric.json` written. On `pass`: phase 8 marked complete. On `fail`: pipeline stops; rubric record names the failures.
