...
Runs `nix develop --command cabal test` and writes the output to `.integration-pipeline/final-test.log`. Non-zero exit aborts the phase.
...
2. Run `nix develop --command cabal test` redirecting to `.integration-pipeline/final-test.log` → verify: log written.
...
1. Run: `nix develop --command cabal test > .integration-pipeline/final-test.log 2>&1`.
...
`.integration-pipeline/final-test.log` written; exit code propagated.
