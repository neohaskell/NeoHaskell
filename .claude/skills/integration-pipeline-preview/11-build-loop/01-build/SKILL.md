...
Runs `nix develop --command cabal build all` and writes the output to `.integration-pipeline/build.log`.
...
2. Run `nix develop --command cabal build all` redirecting stdout+stderr to `.integration-pipeline/build.log` → verify: log file exists.
...
- `.integration-pipeline/` was created during phase 01.
...
3. Run: `nix develop --command cabal build all > .integration-pipeline/build.log 2>&1`.
4. Capture the exit code.
5. If exit is non-zero, surface the tail of `.integration-pipeline/build.log` and exit non-zero.
6. If exit is zero, exit 0.

## Output

`.integration-pipeline/build.log` written; exit code propagated.
