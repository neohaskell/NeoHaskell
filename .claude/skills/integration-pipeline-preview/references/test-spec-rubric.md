...
The review-quality agent reads the produced test spec at `.integration-pipeline/integration-tests.md` plus the architecture doc at `.integration-pipeline/integration-architecture.md`. For each question:
...
1. **Coverage per public function.** Every public function listed in the architecture doc has **at least three** test cases in the spec — minimum 1 happy-path + 2 non-happy (edge / error). Functions with zero, one, or two cases fail this check. The 3-case floor is per-function and absolute; aggregate averages (e.g. TextSpec landing around 2.7 cases per function across its surface) are descriptive context for sibling specs, not an escape hatch — every individual function must still meet the 3-case minimum.
