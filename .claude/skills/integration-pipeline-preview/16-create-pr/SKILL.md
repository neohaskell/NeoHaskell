...
Generates the final PR body and title, commits and pushes the implementation, then converts the **draft PR opened in phase 3** to ready for review. Phase 3 already opened the draft PR (for the design review); this phase updates its title/body and marks it ready — it never creates a second PR.
...
Walk these steps in order. After step 2, PAUSE — wait for explicit maintainer approval before phase 17 runs (CI cycle).
