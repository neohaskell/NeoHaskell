...
Reads grounded findings on stdin, writes `.integration-pipeline/findings-04.json` with aggregate counts, registers it via the pipeline state machine, and marks phase 4 complete. All work happens inside `scripts/record-findings.py` — this leaf just routes input into the script.
...
1. Invoke `python3 .claude/skills/integration-pipeline-preview/scripts/record-findings.py --phase 4 --severity-scheme security`, piping in the grounded findings JSON on stdin (or pass `--input <path>` if the prior step wrote to a file).
...
`.integration-pipeline/findings-04.json` written with `total_findings`, `blockers`, `kept`, `demoted`, `framework_debt`, `findings`. Phase 4 marked complete; pipeline state advanced.
