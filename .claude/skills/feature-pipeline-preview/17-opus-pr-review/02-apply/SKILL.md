...
2. For each accepted finding, in file-path order:
   1. Apply `proposed_fix` per the `kind` (edit / delete / rename / add).
   ...
   4. `git add` the touched files; `git commit -m "<type>(<scope>): <rule>"` with a short body referencing the finding's `explanation`.
   5. If build or test fails: REVERT the working tree changes for this fix only (`git checkout -- .`), then refuse with the finding's `rule` and surface the failure.
