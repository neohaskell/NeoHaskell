...
2. The feature classification at `.pipeline/classification.json` (one of: `trivial`, `simple`, `moderate`, `complex`, `security-critical`).
...
The grounding pass emits a structured record per finding, written by `03-record/SKILL.md` (or `04-record/SKILL.md` for the impl reviews) to `.pipeline/findings-{phase}.json`:
