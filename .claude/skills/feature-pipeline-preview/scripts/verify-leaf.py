...
Re-reads the relevant `.pipeline/` artefacts for the given phase and exits 0
if the leaf's claims match reality, else nonzero with an explanation.

Covered phases:
    11  build loop          — `.pipeline/test.log` shows `0 failures` across suites
    12  security impl       — `.pipeline/findings-12.json` blocker count matches state
    13  performance impl    — `.pipeline/findings-13.json` blocker count is countable
    14  fix findings        — both findings-12 and findings-13 show 0 blockers
    15  final verify        — `.pipeline/test.log` shows `0 failures`; hlint.log exists
    17  Opus PR review      — `.pipeline/findings-17.json` exists with a JSON array
    18  CI cycle            — PR is merged (delegated to gh; not re-verifiable here)
...
def verify_opus_review() -> int:
    path = PIPELINE_DIR / "findings-17.json"
    if not path.exists():
        return fail("phase 17: findings-17.json missing")
    try:
...
def verify_ci() -> int:
    # Cannot reliably check merge state from here; the maintainer's `approve 18`
    # already implies merge-readiness was confirmed externally.
    return ok("phase 18: CI/merge verification delegated to gh status checks")
