...
3. Otherwise run: `hlint <files...> > .pipeline/hlint.log 2>&1 || true` — the trailing `|| true` keeps the leaf at exit 0 regardless of warning count.
...
- `hlint` not on PATH → refuse: "hlint not found" and exit non-zero. Environment failure, not a style warning.
