...
- **`Uuid.generateV5 ns name`** — called from a user command's `getEntityId`
  (once per command intake) and/or from `decide`. Cost is one UTF-8 encode of
  `name`, one `[Word8]` list build, and one SHA-1 over `16 + |name|` bytes.
  For a natural key of realistic size (a repo path, an OAuth subject — tens of
  bytes) that is a single sub-microsecond SHA-1 block, ~3 orders of magnitude
  under the 1ms intake budget.
...
|---|-----------|---------|-----------|---------|
| 1 | P1 Hot-path placement | `generateV5` sits on command intake when a command derives its stream id from a natural key. One UTF-8 encode + one `[Word8]` build + one SHA-1 over a short input ≈ sub-µs against a <1ms budget. `generateDeterministicUuid` adds a `Return` node to a `Decision` the executor already walks. Zero cost for commands that don't call either. | kept | informational (no regression) |
...
| 3 | P5 Allocation — double derivation | A command that derives the **same** UUID in both `getEntityId` and `decide` pays the encode+SHA-1 twice per request. This is a call-site pattern, not a property of the promised API, and the framework cannot memoize it (the two hooks are invoked independently, with no shared context). The haddock should show the id being derived once and reused where both hooks need it; the testbed demo (C8) derives distinct values in the two hooks, so it does not model the wasteful shape. | kept (Q1: 2× a sub-µs cost — real but immaterial; Q3: **not** framework-absorbable) | advisory (documentation, not code) |
...
| 8 | P7 Evidence discipline | The spec makes **no** performance claim, so nothing needs measurement to stand up. Input length is the only cost driver, and it is bounded upstream by the transport's request-body size limit (ADR-0019) — an unbounded-name intake blowup is not reachable through the web transport. No new `telemetry/bench-budgets.json` entry is proposed: a pure sub-µs function with no shared state has no meaningful budget to defend, and the existing command-intake benches already cover the path it sits on. Revisit only if a consumer reports deriving over large payloads. | kept | informational |
...
The change is neutral on the 50k req/s budget: opt-in, pure, sub-µs, allocation
bounded by key length and immediately collected, and zero cost to every command
that does not use it. Measurement stays the nightly bench harness's job, not a
PR gate.
