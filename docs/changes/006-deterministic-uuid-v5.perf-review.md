...
- **`Uuid.generateV5 ns name`** — called from a user command's `getEntityId`
  ...
  bytes) is 1–2 blocks; the cost stays microsecond-scale against the 1ms intake
  budget, and grows linearly from there. **The bound is the caller's**: this is
