...
|----|----------|--------------|-------|----------|
| C1 | Two Postgres stores built with **distinct** query names, writing the **same** instance UUID with different state, each read back their **own** state — no cross-query overwrite (the collision the issue reports) | `hspec:nhcore-test-service:core/test-service/Service/QueryObjectStore/PostgresSpec.hs#distinct query names isolate rows for the same instance uuid (#734)` | integration | postgres:real |
| C2 | `getAll` on a store returns only the rows written under **its own** query name, not rows a different query wrote for the same table | `hspec:nhcore-test-service:core/test-service/Service/QueryObjectStore/PostgresSpec.hs#getAll is scoped to the store's own query name (#734)` | integration | postgres:real |
| C3 | Single-query behavior preserved — `get` / `atomicUpdate` (insert, overwrite, delete) / `getAll` still round-trip correctly once state is keyed by the threaded query name | `hspec:nhcore-test-service:core/test-service/Service/QueryObjectStore/PostgresSpec.hs#state round-trips under the threaded query name` | integration | postgres:real |
| C4 | The **automatic wiring** passes the query's own name to the store factory — `createDefinitionWithStore` supplies `NameOf query` (not `"__trait__"` or empty) to the supplied factory, so the correct name reaches a Postgres store in production wiring, not only in manually-constructed stores. A spy factory captures the name it is handed | `hspec:nhcore-test-service:core/test-service/Service/Query/DefinitionSpec.hs#passes NameOf query to the supplied store factory (#734)` | unit | none |
| C5 | `getAll` excludes the reserved **nil-UUID checkpoint marker** row. Because the fix makes the trait's per-instance state and the checkpoint marker (`Subscriber.rebuildFrom` via `CheckpointStore`) share one `query_name`, `getAll` must return only real instances — never the marker's placeholder state, which would otherwise surface a bogus row at `GET /queries/{name}` for any checkpointed query | `hspec:nhcore-test-service:core/test-service/Service/QueryObjectStore/PostgresSpec.hs#getAll excludes the reserved nil-uuid checkpoint marker row (#734)` | integration | postgres:real |
| C6 | **Concurrent** read-modify-write `atomicUpdate` on the **same** `(query_name, instance_uuid)` loses no updates: N interleaved increments of one counter row all apply (final == N). Proves `atomicUpdate` is genuinely atomic (optimistic compare-and-set with bounded retry) rather than a racy read-then-write across separate pool sessions — the lost-update defect CodeRabbit flagged on PR #739 | `hspec:nhcore-test-service:core/test-service/Service/QueryObjectStore/PostgresSpec.hs#N concurrent read-modify-write increments on one key all apply (#739)` | integration | postgres:real |
