...
Reads the produced architecture doc and the design draft, applies every check in `../../references/architecture-rubric.md`, records the per-check verdict to `.integration-pipeline/architecture-rubric.json`, and gates phase 7 completion on the result.
...
Assume the artefact under review was produced by a language model (ChatGPT-class output). Treat plausible-looking claims as unverified, expect hallucinated APIs and missed constraints, and refuse to pass anything not directly traceable to the rubric in `../../references/architecture-rubric.md` and the source design draft. Strict review is the default — benefit of the doubt goes to the rubric and to the source, never to the producer.

## Inputs

- `.integration-pipeline/integration-architecture.md` — produced by `../01-produce/`.
- `.integration-pipeline/integration-design.md` — the source design draft.
...
2. For each of the eight rubric checks, decide `pass` / `fail` / `n/a` from evidence in the architecture doc vs the design draft → verify: every check has a verdict + an evidence cite (section + line).
3. For each of the four carrier rules, decide `pass` / `fail` → verify: every rule has a verdict.
4. Write the rubric record to `.integration-pipeline/architecture-rubric.json` → verify: file exists with `checks`, `carriers`, `verdict`.
...
1. Load the architecture doc, the design draft, the rubric, and the nhcore-context reference.
2. Walk each rubric check 1-8 in order:
   ...
   - Check 6 (errors enumerated): every error case from the design draft appears as a constructor in the doc.
...
5. Write `.integration-pipeline/architecture-rubric.json`.
6. On `pass`: run `python3 .claude/skills/integration-pipeline-preview/scripts/pipeline.py complete 7`; print `RUBRIC: pass`.
...
`.integration-pipeline/architecture-rubric.json` written. On `pass`: phase 7 marked complete. On `fail`: pipeline stops; rubric record names the failures.
