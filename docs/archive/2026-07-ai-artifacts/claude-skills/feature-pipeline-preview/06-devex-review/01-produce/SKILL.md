...
---
name: 01-produce
description: Produces the DevEx review document for the feature's ADR.
kind: leaf
executor: opus
...
- `.pipeline/findings-04.json` — grounded security findings.
- `.pipeline/findings-05.json` — grounded performance findings.
...
Assumptions:
- The ADR uses the NeoHaskell ADR template and has Status: `Proposed` or `Accepted`.
...
5. Write `.pipeline/devex-review.md` with these sections:
   ...
   - `## Action items` — concrete edits the ADR needs before phase 6 can advance.
6. Print the path to the written file. Do not run `pipeline.py complete`. The review-quality step owns that call.

## Output

`.pipeline/devex-review.md` exists with the four sections above. Stdout is a single line: `wrote .pipeline/devex-review.md`.

## Refusals

- Any prerequisite input missing → refuse with the missing path.
- The ADR's Public API section is empty → refuse: "ADR has no public API to review".
