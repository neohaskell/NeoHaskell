...
3. Always exit 0 — the leaf records warnings, the PR body surfaces them, and CI's own hlint check is the canonical gate at merge time. When hlint is unavailable, write a placeholder line (`hlint: missing`) to `.pipeline/hlint.log` and still exit 0 so the phase-complete check (`.pipeline/hlint.log` refreshed) has a single canonical signal → verify: caller sees exit 0, `.pipeline/hlint.log` always exists.
