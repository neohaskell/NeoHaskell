...
These pragma-free modules require the CLI preset fix that enables
`DerivingStrategies` for generated projects. The published Neo CLI `0.10.0`
does not include that fix. Local verification uses the corrected, unreleased CLI
with framework revision `25bd7027a85b8f2999602f66b46b4b6b133c22e8`, the pin
generated by `0.10.0`. Keep the application's `neo-version` pin while comparing
the checkpoint; see the installation chapter for release availability.
