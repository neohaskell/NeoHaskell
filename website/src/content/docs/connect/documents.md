---
title: Read documents and audio
description: Turn attachments into reviewable information without treating extracted text as fact.
sidebar:
  order: 6
---

A supplier sends a PDF describing new mugs. Jess wants help extracting names and dimensions before updating the catalogue. The useful result is a reviewable draft: the original attachment remains the evidence, and the shop decides which extracted information becomes product data.
...
Prerequisites: [file uploads](/connect/files/), [integration lifecycle](/connect/), and an outcome command for recording results.
...
A scanned page may have no selectable text. Local PDF extraction is not OCR. Check empty output before recording a successful business extraction.
...
The configuration offers `FullText`, `Summary`, and `Structured` extraction modes. `Structured` changes the prompt; it does not turn the returned `Text` into a validated product schema. Parse the result and apply the same product rules you would apply to human input.
...
For recorded catalogue notes, `Integration.Audio.Transcribe.Request` uses the same file-reference pattern. Import `Integration.Audio.Transcribe.Internal ()` for execution. Its configuration includes a language hint and `maxDurationSeconds`; the latter asks the model to limit transcription but still uploads the full file.
...
Ask the agent to draft a workflow where extracted dimensions require merchant approval before publication.
...
Next, [use AI for application features](/connect/ai/) with the same separation between suggestion and accepted business facts.
