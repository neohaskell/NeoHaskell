---
title: Measure the work customers care about
description: Investigate latency, replay, contention, and database budgets with representative shop workloads.
sidebar:
  order: 7
---

The shop feels fast with one order. A busy sale introduces many customers, a popular product, and a much longer history. Performance work begins by choosing which experience must remain acceptable: accepting an order, showing its status, or restarting after a deployment.
...
Write a target and a workload together. “Order status appears within our agreed time while several customers reserve the last mugs” is testable. “The framework is fast” is not.
...
## Understand contention on one product

Two customers may try to reserve the final mug from the same stock entity. Optimistic concurrency detects conflicting writes, and the command executor can refetch state and retry its decision.
...
Ask whether one entity contains unnecessarily unrelated activity. Splitting independent products can reduce contention, but splitting one indivisible business invariant may make correctness harder. Preserve the rule you are trying to enforce.
...
That script intentionally truncates and drops test tables. Run it only through the documented contributor test environment with a disposable database; do not aim it at your shop's database. Read [the public test](https://github.com/neohaskell/NeoHaskell/blob/main/testbed/scripts/cold-start-readiness.sh) to understand what it proves.

For your own load test, use representative order histories and avoid real payment or email effects. Check final stock and order totals as well as latency. Fast incorrect results are a failed test.

## Exercise: the final two mugs

Run simultaneous attempts to buy the last two mugs under the reservation policy you implemented. Decide the expected successful count before running the test.
