...
description: "Plain-language meanings for NeoHaskell concepts, with links to practical examples."
sidebar:
  order: 2
---

Use this page when a term gets between you and the application behaviour you are
trying to understand. Definitions apply across domains. Examples from the recurring
ecommerce practice project make some of them concrete; the links lead to deeper explanations.

| Term | Meaning and example | Read more |
| --- | --- | --- |
...
| Event modeling | Describing how requests, facts, views, and external actions connect | [Event modeling](/start/event-modeling/) |
...
| Integration | An explicit connection to another part of the application or an external service | [Integrations](/connect/) |
...
**A request is not a fact.** In the ecommerce example, “Charge the customer” is a request. A confirmed payment
is evidence of something that happened. A timeout leaves uncertainty; it does not
establish that no charge occurred.

**State is not a view for every reader.** An entity's state supports decisions.
Different readers may need different views and access rules. In the ecommerce
example, a customer's summary and a merchant's report serve different purposes.
