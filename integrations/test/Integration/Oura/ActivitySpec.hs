module Integration.Oura.ActivitySpec (spec) where
...
spec = do
  describe "DailyActivity" do
    it "can be constructed with all fields" do
      ...
      let request = DailyActivity userId startDate endDate onSuccess onError
    ...
    it "can be constructed without error callback" do
      ...
      let onSuccess = \_ -> "success" :: Text
      let request = DailyActivity userId startDate endDate onSuccess Nothing
      request.userId `shouldBe` userId

    it "stores date range correctly" do
      let request = DailyActivity "user-789" "2024-03-01" "2024-03-31" (\_ -> ()) Nothing
