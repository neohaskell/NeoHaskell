   ...
   data OuraHttpError
     ...
     | RateLimited Int        -- 429: rate limit hit, Int = suggested retry-after seconds (default 60)
   ...
   realHttpFetch accessToken url = do
     ...
     case response.statusCode of
       ...
       429 -> do
         ...
         let retryAfter = response.headers
               |> Array.find (\(k, _) -> k == "Retry-After")
               |> Maybe.map (\(_, v) -> Text.toInt v |> Maybe.withDefault 60)
   ...
   executeDailySleep httpFetch ctx config = do
     ...
     let baseUrl = [fmt|https://api.ouraring.com/v2/usercollection/daily_sleep?start_date=#{config.startDate}&end_date=#{config.endDate}|]
     -- Execute with token refresh and error handling
     fetchResult <- withValidToken
       ctx.secretStore
       "oura"
       config.userId
       (OAuth2.refreshTokenValidated providerConfig.validatedProvider providerConfig.clientId providerConfig.clientSecret)
       isUnauthorized
       (\accessToken -> executeDailySleepInternal httpFetch accessToken baseUrl)
       |> Task.asResult
     -- Handle result with onError callback
     case fetchResult of
       Ok result -> Integration.emitCommand (config.onSuccess result)
       Err tokenErr -> do
   ...
   executeDailyActivity httpFetch ctx config = do
     providerConfig <- getOuraProvider ctx
     let baseUrl = [fmt|https://api.ouraring.com/v2/usercollection/daily_activity?start_date=#{config.startDate}&end_date=#{config.endDate}|]
     fetchResult <- withValidToken ctx.secretStore "oura" config.userId
       (OAuth2.refreshTokenValidated providerConfig.validatedProvider providerConfig.clientId providerConfig.clientSecret)
       isUnauthorized
   ...
   executeDailyReadiness httpFetch ctx config = do
     providerConfig <- getOuraProvider ctx
     let baseUrl = [fmt|https://api.ouraring.com/v2/usercollection/daily_readiness?start_date=#{config.startDate}&end_date=#{config.endDate}|]
     fetchResult <- withValidToken ctx.secretStore "oura" config.userId
       (OAuth2.refreshTokenValidated providerConfig.validatedProvider providerConfig.clientId providerConfig.clientSecret)
       isUnauthorized
       (\accessToken -> fetchAllPages httpFetch accessToken baseUrl)
   ...
   executeHeartRate httpFetch ctx config = do
     ...
     let startEncoded = urlEncodeDatetime config.startDatetime
     let endEncoded = urlEncodeDatetime config.endDatetime
   ...
   getOuraProvider ctx = case Map.get "oura" ctx.providerRegistry of
   ...
   isUnauthorized err = case err of
     Unauthorized -> True
     RateLimited _ -> False  -- Don't retry on rate limit
   ...
   mapTokenError err = case err of
     ...
     ActionFailed (RateLimited retryAfter) -> Integration.RateLimited retryAfter  -- Re-use existing IntegrationError constructor
   ...
   fetchPagesLoop httpFetch accessToken baseUrl maybeNextToken accumulated = do
     ...
     let newAccumulated = accumulated |> Array.append paginated.responseData
     case paginated.nextToken of
