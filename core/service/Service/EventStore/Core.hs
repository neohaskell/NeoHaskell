...
data EventStore eventType = EventStore
  { -- | Insert an event in a stream
    ...
    readStreamForwardFrom :: EntityName -> StreamId -> StreamPosition -> Limit -> Task Error (Array Event),
    -- | Read events from a stream in backward direction starting from a given revision.
    --   Useful for looking at recent events.
    readStreamBackwardFrom :: EntityName -> StreamId -> StreamPosition -> Limit -> Task Error (Array Event),
    -- | Read all events from a specific stream for a given entity.
    --   Returns an array of events or an Error.
    readAllStreamEvents :: EntityName -> StreamId -> Task Error (Array Event),
    ...
    readAllEventsForwardFromFiltered :: StreamPosition -> Limit -> Array EntityName -> Task Error (Array Event),
    -- | Read events from the global stream in backward direction starting from a given global position,
    --   but only for specific entities. Useful for filtered historical analysis and debugging.
    readAllEventsBackwardFromFiltered :: StreamPosition -> Limit -> Array EntityName -> Task Error (Array Event),
    ...
    subscribeToEntityEvents :: EntityName -> (Event -> Task Error Unit) -> Task Error SubscriptionId,
    ...
    subscribeToStreamEvents :: EntityName -> StreamId -> (Event -> Task Error Unit) -> Task Error SubscriptionId,
