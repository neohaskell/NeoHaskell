...
decode (INTERNAL_CORE_CODEC_CONSTRUCTOR parser) _ =
  Promise.do
    print "Decoding"
    let opts =
          OptParse.info
    ...
    -- )
    print "Getting args"
    args <- System.getArgs |> Promise.fromIO
    let result = OptParse.execParserPure OptParse.defaultPrefs opts (spy "args" args)
    case result of
      OptParse.Success value ->
        Promise.yield value
      OptParse.Failure failure ->
        unsafelyPanicAndDie (Prelude.fst (OptParse.renderFailure failure ("" |> Convert.toLegacy)) |> Convert.fromLegacy)
      OptParse.CompletionInvoked _ ->
        todo
        ...
        if not props.isHidden
          then Optional.None
            ...
            OptParse.hidden
              |> Optional.Some
  let optionalOptions =
        [nameOption, descriptionOption, shorthandOption, hiddenOption]
  let options = Array.dropNones optionalOptions

  let mod =
        Compat.toList options
          |> Monoid.mconcat
  INTERNAL_CORE_CODEC_CONSTRUCTOR (OptParse.switch mod)
